...
std::map<lua_State*, ROC::LuaFunction::LuaFunctionMap*> ROC::LuaFunction::ms_vmFuncMap;

int ROC::LuaFunction::RetrieveGlobalReference(lua_State *f_vm, void *f_ptr)
{
    int l_ref = 0;
...
void ROC::LuaFunction::RemoveGlobalReference(lua_State *f_vm, void *f_ptr, int f_ref)
{
    auto l_funcMapIter = ms_vmFuncMap.find(f_vm);
...
ROC::LuaFunction::LuaFunction()
{
    m_vm = nullptr;
    m_ptr = nullptr;
    m_ref = 0;
}
ROC::LuaFunction::LuaFunction(const LuaFunction &f_func)
{
    if(f_func.IsValid())
    {
        m_vm = f_func.m_vm;
...
ROC::LuaFunction& ROC::LuaFunction::operator=(const LuaFunction& f_func)
{
    if(f_func.IsValid())
    {
        m_vm = f_func.m_vm;
...
bool ROC::LuaFunction::operator==(const LuaFunction& f_func) const
{
    return ((m_vm == f_func.m_vm) && (m_ref == f_func.m_ref));
}

void ROC::LuaFunction::Retrieve(lua_State *f_vm, void *f_ptr)
{
    if(!IsValid())
    {
        m_vm = f_vm;
