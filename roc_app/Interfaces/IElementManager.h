...
public:
    ...
    virtual ICamera* CreateCamera(int f_type) = 0;
    ...
    virtual IRenderTarget* CreateRenderTarget(int f_type, const glm::ivec2 &f_size, int f_filter) = 0;
    virtual ITexture* CreateTexture(const std::string &f_path, int f_type, int f_filter, bool f_compress) = 0;
    virtual ITexture* CreateTexture(const std::vector<std::string> &f_path, int f_filter, bool f_compress) = 0;
    virtual IFont* CreateFont_(const std::string &f_path, int f_size, const glm::ivec2 &f_atlas, int f_filter) = 0;
    virtual IFile* CreateFile_(const std::string &f_path) = 0;
    virtual IFile* OpenFile(const std::string &f_path, bool f_ro) = 0;
    virtual ICollision* CreateCollision(int f_type, const glm::vec3 &f_size, float f_mass) = 0;
