...
int LuaDrawableDef::Draw(lua_State *f_vm)
{
    ...
    {
        bool l_result = Core::GetInstance()->GetEngineICore()->GetIRenderManager()->Draw(l_drawable, l_pos, l_size, l_rot, l_color);
...
int LuaDrawableDef::Draw3D(lua_State *f_vm)
{
    ...
    if(!l_argStream.HasErrors())
    {
        ...
        bool l_result = Core::GetInstance()->GetEngineICore()->GetIRenderManager()->Draw(l_drawable, l_pos, l_rot, l_size, l_params);
        l_argStream.PushBoolean(l_result);
    }
    else l_argStream.PushBoolean(false);
    return l_argStream.GetReturnValue();
