...
TaskHandler::TaskHandler(Core *f_module)
{
...
{
    void* l_task = m_luaModule->GetEngineICore()->GetIAsyncManager()->LoadGeometry(f_path);
...
{
    void* l_task = m_luaModule->GetEngineICore()->GetIAsyncManager()->LoadTexture(f_path, f_type, f_filter, f_compress);
    m_taskMap.emplace(l_task, f_func);
    return l_task;
}
...
{
    void* l_task = m_luaModule->GetEngineICore()->GetIAsyncManager()->LoadTexture(f_path, f_filter, f_compress);
    m_taskMap.emplace(l_task, f_func);
    return l_task;
}
